...
_mainScript_() {
    ...
    if command -v poetry &>/dev/null; then
        pushd "/workspaces/obsidian-frontmatter" &>/dev/null
    ...
    if command -v pre-commit &>/dev/null; then
        if [ -d "/workspaces/obsidian-frontmatter/.git" ]; then
            pushd "/workspaces/obsidian-frontmatter" &>/dev/null
            _execute_ -pv "pre-commit install --install-hooks"
            _execute_ -pv "pre-commit autoupdate"
            popd &>/dev/null
        else
            warning "Git repository not found in /workspaces/obsidian-frontmatter. Initialize pre-commit manually."
        fi
    else
